...
#//Final ans//
select origin, destination, flight.flight_id, round(avg(fare),2) as "Avg_fare_paid_per_booking" 
	from booking 
	join flight on booking.flight_id= flight.flight_id 
where flight.status="flown" and year(flight.flight_date)=2019 and month(flight.flight_date)=01 
group by flight.flight_id , origin, destination;
...
select origin, destination, 
	   round(sum(fare),2)/count(distinct(flight.flight_id)) as " Avg_fare_paid_per_flight" 
       from flight 
	   join booking on booking.flight_id= flight.flight_id 
where flight.status="flown" and year(flight.flight_date)=2019 and month(flight.flight_date)=01
...
select ff.customer_id, count(td.booking_id) as "number of flights booked",
	   ff.d as " Fisrt booking date" ,ff.d+ interval 364 day as 365th_day_from_firstbooking
			from booking as td 
       ...
       and td.booking_date between ff.d and date_add(ff.d,interval 364 day)
group by ff.customer_id;

#// step 2: Now add this new column first_booking_date to booking table// 
select * from 
		booking as td right join fb on td.customer_id=fb.customer_id 
		and td.booking_date between fb.first_booking_date and date_add(fb.first_booking_date,interval 365 day);

#// step3: Now get each customer and number of bookings by them in their first year. ("RUN CTE and this step togather")//
select fb.customer_id, count(td.booking_id) as "number of flights booked",fb.first_booking_date
	from booking as td 
    right join fb on td.customer_id=ff.customer_id
	and td.booking_date between fb.first_booking_date and date_add(fb.first_booking_date,interval 365 day)
